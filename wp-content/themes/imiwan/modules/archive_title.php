...
$title = get_the_title();
$content = mb_strimwidth(strip_tags(apply_filters('the_content', $post->post_content)), 0, 160,"......",'utf-8');//300是摘要字符数，......是结束符号。
if($s){$keys = explode(" ",$s);
...
<article class="excerpt<?php echo !$_thumbnail ? ' excerpt-nothumbnail' : '' ?>">
	...
	<header>
		...
		<span class="note"><?php echo deel_strimwidth(strip_tags(apply_filters('the_content', $post->post_content)), 0, 80, '...'); ?></span>
<p class="auth-span">

...
if(is_admin()){
	...
	//add_action('plugins_loaded', create_function( '$a', "define('WP_POST_REVISIONS', false);"));
		add_action('plugins_loaded', create_function( '$a', "remove_action('post_updated','wp_save_post_revision' );"));//新版
	endif;
...
if (isset($youpzt_optimize_options['gravatar-replace']) && $youpzt_optimize_options['gravatar-replace'] ==3){
	...
	function youpzt_get_qiniu_avatar($avatar) {//secure
	$avatar = str_replace(array("www.gravatar.com", "0.gravatar.com", "1.gravatar.com", "2.gravatar.com"), "dn-avacdn.qbox.me", $avatar);
	  return $avatar;
	}
